...
if kubectl >/dev/null 2>&1; then
    unset KUBECONFIG
    kubectl config set-cluster ucp_ucp.docker.ee:6443_shaker --server https://ucp.docker.ee:6443 --certificate-authority "$PWD/ca.pem" --embed-certs
    kubectl config set-credentials ucp_ucp.docker.ee:6443_shaker --client-key "$PWD/key.pem" --client-certificate "$PWD/cert.pem" --embed-certs
